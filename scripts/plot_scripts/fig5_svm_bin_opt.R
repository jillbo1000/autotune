...
bc.dat <- get_opt_data("./data/optimization_data/svm/Binary/Breast Cancer/", 
                       "Breast Cancer")
io.dat <- get_opt_data("./data/optimization_data/svm/Binary/Ionosphere/", 
                       "Ionosphere")
li.dat <- get_opt_data("./data/optimization_data/svm/Binary/Lichen/", 
                       "Lichen")
mu.dat <- get_opt_data("./data/optimization_data/svm/Binary/Mullein/",
                       "Mullein")
pi.dat <- get_opt_data("./data/optimization_data/svm/Binary/Pima/", 
                       "Pima")
so.dat <- get_opt_data("./data/optimization_data/svm/Binary/Sonar/", 
                       "Sonar")
...
# Add in 1s for the missing Mullein data
...
# Remove the results for the two stage test
pcp_sum <- pcp_sum[pcp_sum$Optimizer != "two", ]
pcp_sum2 <- pcp_sum2[pcp_sum2$Optimizer != "two", ]
...
pdf("./plots/fig5_svm_bin_opt.pdf", height = 4.3, width = 8)
ggplot(pcp, aes(x = Optimizer, y = Performance, group = cat)) +
  geom_path(aes(color = factor(cat)), alpha = 0.7, lwd = 1.2) +
  scale_color_manual("", values = cols) +
  labs(title = "SVM Binary Classification", x = NULL, y = "Performance") +
