...
bc.dat <- get_opt_data("./data/optimization_data/gbm/Binary/Breast Cancer", 
                       "Breast Cancer")
io.dat <- get_opt_data("./data/optimization_data/gbm/Binary/Ionosphere/", 
                       "Ionosphere")
li.dat <- get_opt_data("./data/optimization_data/gbm/Binary/Lichen/", 
                       "Lichen")
mu.dat <- get_opt_data("./data/optimization_data/gbm/Binary/Mullein/",
                       "Mullein")
pi.dat <- get_opt_data("./data/optimization_data/gbm/Binary/Pima/", 
                       "Pima")
so.dat <- get_opt_data("./data/optimization_data/gbm/Binary/Sonar/", 
                       "Sonar")
...
mu_test <- pcp_sum$Optimizer[pcp_sum$cat == "Mullein"]

# Add in some values for the missing Mullein data

mullein_tmp <- as.data.frame(cbind(unique(pcp_sum$Optimizer), "Mullein", 
                                   matrix(1, ncol = 8, nrow = length(unique(pcp_sum$Optimizer)))))
colnames(mullein_tmp) <- colnames(pcp_sum)
mullein_tmp <- mullein_tmp[!(mullein_tmp$Optimizer %in% mu_test), ]
...
pdf("./plots/fig6_gbm_bin_opt.pdf", height = 4.3, width = 8)
ggplot(pcp, aes(x = Optimizer, y = Performance, group = cat)) +
  geom_path(aes(color = factor(cat)), alpha = 0.7, lwd = 1.2) +
  scale_color_manual("", values = cols) +
  labs(title = "GBM Binary Classification", x = NULL, y = "Performance") +
